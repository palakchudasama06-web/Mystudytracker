...
if st.session_state.user_id is None:
    ...
    <div class="hero">
        ...
        <p>A professional study command center built around articleship, CA Final, revision and consistency.</p>
    </div>
    """, unsafe_allow_html=True)
...
if st.session_state.page == "Dashboard":
    ...
    cards = [
        ...
        ("⭐", "LEVEL", f"Level {level}", f"{current_xp:,} / {next_xp:,} XP"),
        ("⏱️", "TODAY", f"{today_hours:.1f} hrs", "Focused study time"),
        ("📅", "THIS WEEK", f"{week_hours:.1f} hrs", "Total study time"),
    ]
    ...
    st.markdown('<div class="section-title">🔄 Revisions Due</div>', unsafe_allow_html=True)
    due = get_due_revisions(user["id"])
    if due:
        for r in due[:5]:
            st.warning(f"**{r['chapter_name']}** — {r['revision_type']} revision is due.")
    else:
        st.success("No overdue revisions. Keep the chain alive! 🔥")
    ...
    with q1:
        if st.button("⏱️ Start Pomodoro", use_container_width=True):
            ...
            st.rerun()
    with q2:
        if st.button("📚 Update Syllabus", use_container_width=True):
            go("Subjects & Chapters")
            st.rerun()
    with q3:
        if st.button("🔄 Check Revisions", use_container_width=True):
            go("Revision Center")
            st.rerun()
...
# ---------- Subjects ----------
    ...
    st.caption("Customize the syllabus to match your attempt and your coaching material.")

    subjects = get_subjects(user["id"])

    with st.expander("➕ Add a Subject", expanded=not subjects):
...
# ---------- Planner ----------
    ...
    st.caption("Build a realistic plan around articleship instead of planning an imaginary 12-hour day.")

    with st.form("planner"):
        c1, c2 = st.columns(2)
        with c1:
            articleship_today = st.checkbox("Articleship today", value=bool(user["articleship"]))
            office_hours = st.number_input("Articleship hours", 0.0, 16.0, 8.0 if articleship_today else 0.0, .5)
        with c2:
            available = st.number_input("Study time available today (hours)", .5, 16.0, 2.5 if articleship_today else 5.0, .5)
        ...
        if st.form_submit_button("Generate Today's Plan", use_container_width=True):
            ...
            if not subjects:
                ...
                st.success(f"Plan generated for {available_minutes} minutes.")
                ...
                if energy == "Low":
                    st.info("Low-energy mode: start with revision or question practice before heavy new learning.")
...
# ---------- Pomodoro ----------
    ...
    st.write(f"**{subject} • {duration} minutes**")
    st.info("For a reliable Streamlit deployment, this first version uses a manual completion button. A browser timer can be added next without changing the study database.")
...
# ---------- Settings ----------
    ...
    st.markdown("### Personalization ideas")
    st.info("The architecture is ready for future settings such as daily target, preferred study hours, motivational tone, focus durations, theme, and custom CA subjects.")
